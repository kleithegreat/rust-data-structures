...
mod tests {
    ...
    fn test_with_complex_types() {
        ...
        let obj1 = TestStruct { a: 1, b: String::from("one") };
        let obj2 = TestStruct { a: 2, b: String::from("two") };
        
        list.push_front(obj1.clone());
        list.push_back(obj2.clone());
        
        assert_eq!(list.size(), 2);
        assert_eq!(list.peek_front(), Some(&obj1));
        assert_eq!(list.peek_back(), Some(&obj2));
        
        assert_eq!(list.pop_front(), Some(obj1));
        assert_eq!(list.pop_back(), Some(obj2));
        assert!(list.is_empty());
    }
